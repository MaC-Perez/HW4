...
There is a close relationship between size and P/B; the bigger animals are, the lower the P/B. Here we have: Whales: P/B = 0.05year−1; seals: get F from catch, and M is 0.09 year−1; shrimp P/B = 3year−1; benthos P/B = 3year−1; zooplankton: it is mainly small Acartia-sized plankton, with P/B = 35year−1.
...
**Increasing on fishing effort for seals, reduce the seals population and increase cod and whiting biomass due to less predation mortality**

```{r}
#load(here('data', 'AB.params.rda'))
#load(here('data', 'AB.rda'))
AB.base <- rsim.scenario(AB, AB.params, 1:25)
AB.base <- adjust.fishing(AB.base, parameter = 'ForcedEffort', group = 'sealers',
...
```
```{r}
cod <- extract.node(AB.run, 'cod')
plot(cod$Biomass, xlab = 'Month', ylab = 'Biomass')

whiting <- extract.node(AB.run, 'whiting')
plot(whiting$Biomass, xlab = 'Month', ylab = 'Biomass')
```
**on the other hand, a decrease in fishing effort on seals will produce higher seals biomass due to less fishing pressure, producing a decrease in cod and whiting biomass**
...
if I decrease fishing effort on trawler fishery I will have a higher biomass of cod and groundfish species
