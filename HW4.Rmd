...
```{r, include=FALSE}
AB.base <- rsim.scenario(AB, AB.params, 1:25)
AB.base7 <- adjust.fishing(AB.base, parameter = 'ForcedEffort', group = 'trawlers',
                          sim.year = 1:25, value = 1)
```

```{r, include=FALSE}
AB.run7 <- rsim.run(AB.base7, years = 1:25)

#Output results
AB.run7
#To capture results as an object use the write.Rsim function
AB.output7 <- write.Rsim(AB.run7)
```
...
```{r}
cod <- extract.node(AB.run7, 'cod')
plot(cod$Biomass, xlab = 'Month', ylab = 'Biomass', main = "Cod")

whiting <- extract.node(AB.run7, 'whiting')
plot(whiting$Biomass, xlab = 'Month', ylab = 'Biomass', main = "Whiting")
```
